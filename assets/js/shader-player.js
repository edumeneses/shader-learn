...
(function () {
  ...
  class ShaderPlayer extends HTMLElement {
    ...
    _release() {
      if (!this._gl) return;
      this._pause();
      // A camera stream outlives a released context unless it is stopped here,
      // and the player caps live contexts, so a library page full of shaders
      // would otherwise leave cameras running.
      for (const name of Object.keys(this._imageTextures || {})) {
        this._stopVideo(name);
      }
    ...
    _initGL() {
      ...
      this._imageSources = this._imageSources || Object.create(null);
      for (const input of m.inputs) {
        if (input.type !== "image") continue;
        if (!this._imageSources[input.name]) this._imageSources[input.name] = "testcard";
        this._loadImageSource(input.name, this._imageSources[input.name]);
      }
    ...
    _buildControls() {
      ...
      panel.innerHTML = "";
      const imageInputs = this._manifest.inputs.filter((i) => i.type === "image");
      const inputs = this._manifest.inputs.filter((i) => i.type !== "image");
      if (!inputs.length && !imageInputs.length) {
        panel.hidden = true;
        return;
      }
      ...
      for (const input of imageInputs) {
        panel.appendChild(this._imageControl(input));
      }
    ...
    _imageControl(input) {
      ...
      row.className = "sp-row sp-row-image";
      ...
      for (const [value, text] of [["testcard", "test card"],
                                   ["file", "a file of your own"],
                                   ["webcam", "camera"]]) {
        const opt = document.createElement("option");
        opt.value = value;
      ...
      select.value = this._imageSources[input.name] || "testcard";

      const picker = document.createElement("input");
      picker.type = "file";
      picker.accept = "image/*";
      picker.hidden = select.value !== "file";
      picker.addEventListener("change", () => {
        if (picker.files && picker.files[0]) {
          this._loadImageSource(input.name, "file", picker.files[0]);
        }
      });

      select.addEventListener("change", () => {
        picker.hidden = select.value !== "file";
        if (select.value === "file") picker.click();
        else this._loadImageSource(input.name, select.value);
      });
    ...
    // -- images ---------------------------------------------------------
    //
    // An ISF `image` input needs something to read. The default is the course's
    // test card, which is generated by an ISF shader and committed as a PNG, so
    // the browser and the offline renderer load exactly the same bytes; drawing
    // the card again in canvas 2D would reintroduce the divergence the manifest
    // design exists to prevent.

    _testcardURL() {
      const manifest = this.getAttribute("manifest") || "";
      const base = manifest.replace(/\/shaders\/[^/]*$/, "");
      return base + "/images/testcard.png";
    }

    _ensureTexture(name) {
      const gl = this._gl;
      let entry = this._imageTextures[name];
      if (!entry) {
        const tex = gl.createTexture();
        gl.bindTexture(gl.TEXTURE_2D, tex);
        // One opaque black texel until the real image arrives, so the shader
        // never samples an incomplete texture.
        ...
        gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE);
        gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);
        entry = { tex, width: 1, height: 1, video: null };
        this._imageTextures[name] = entry;
      }
      return entry;
    }

    _uploadImage(name, source, width, height) {
      const gl = this._gl;
      const entry = this._ensureTexture(name);
      gl.bindTexture(gl.TEXTURE_2D, entry.tex);
      // Flip on upload. A texture's origin is bottom left and an image file's
      // is top left, so an unflipped upload is upside down; the offline
      // renderer flips on load for the same reason.
    ...
    _loadImageSource(name, source, file) {
      this._imageSources[name] = source;
      this._stopVideo(name);

      if (source === "webcam") {
        this._openWebcam(name);
        return;
      }

      const url = source === "file" && file ? URL.createObjectURL(file)
                                            : this._testcardURL();
      const img = new Image();
      img.crossOrigin = "anonymous";
      img.onload = () => {
        this._uploadImage(name, img, img.naturalWidth, img.naturalHeight);
        if (source === "file" && file) URL.revokeObjectURL(url);
      };
      img.onerror = () => {
        this._status("could not load " + url, true);
      };
      img.src = url;
    }

    async _openWebcam(name) {
      try {
        ...
        this._status("camera not available: " + err.message, true);
        this._loadImageSource(name, "testcard");
      }
    }

    _stopVideo(name) {
      const entry = this._imageTextures && this._imageTextures[name];
    ...
    /** Push a frame from any live video source into its texture. */
    _updateVideoTextures() {
      for (const name of Object.keys(this._imageTextures)) {
        const entry = this._imageTextures[name];
        if (entry.video && entry.video.readyState >= 2) {
          this._uploadImage(name, entry.video,
            entry.video.videoWidth, entry.video.videoHeight);
        }
      }
    }
